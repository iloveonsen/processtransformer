...
def preprocess_single_trace(input_csv_path, output_csv_path, columns):
    """
    Preprocess a single trace CSV file for next time prediction.
    ...
    for i in range(len(df) - 1):  # Exclude last one as we need next_time
        ...
        # Next time is the time to the next event
        next_time = (timestamps[i+1] - timestamps[i]).total_seconds() / 86400
...
def visualize_attention_scores(activity_scores, dataset_name, task_name, metric_value, output_path):
    ...
    Args:
        ...
        task_name: Name of the task (e.g., "next_time")
...
def run_inference(args):
    ...
    model_path = os.path.join(args.model_dir, args.dataset, "next_time_ckpt.pt")
    ...
    # Get metric value from checkpoint (MAE for next_time)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Process Transformer - Next Time Inference")
